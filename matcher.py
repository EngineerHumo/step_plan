...
def hungarian_match(cost: torch.Tensor, valid_gt_mask: torch.Tensor) -> List[Tuple[torch.Tensor, torch.Tensor]]:
    ...
    for b in range(B):
        ...
        # Track both the column position inside ``cost_slice`` and the original
        # column index so that we always index the correct cost entry even after
        # some columns have been matched and removed from consideration.
        remaining_pairs = [(idx, int(col)) for idx, col in enumerate(cols.tolist())]
        row_match: List[int] = []
        col_values: List[int] = []
        while remaining_rows and remaining_pairs:
            min_val = None
            sel_row = None
            sel_pair_idx = None
            for r in remaining_rows:
                row_vals = cost_slice[r]
                for pair_idx, (slice_idx, col_val) in enumerate(remaining_pairs):
                    val = row_vals[slice_idx].item()
                    if min_val is None or val < min_val:
                        min_val = val
                        sel_row = r
                        sel_pair_idx = pair_idx
            if sel_row is None or sel_pair_idx is None:
                break
            row_match.append(sel_row)
            _, matched_col = remaining_pairs.pop(sel_pair_idx)
            col_values.append(matched_col)
            remaining_rows.remove(sel_row)
            # ``remaining_pairs`` already had the matched column removed; nothing
            # else to update.
        rows_tensor = torch.tensor(row_match, dtype=torch.long, device=cost.device)
        cols_tensor = torch.tensor(col_values, dtype=torch.long, device=cost.device) if col_values else torch.empty(0, dtype=torch.long, device=cost.device)
