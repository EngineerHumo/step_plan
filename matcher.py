...
def hungarian_match(cost: torch.Tensor, valid_gt_mask: torch.Tensor) -> List[Tuple[torch.Tensor, torch.Tensor]]:
    ...
    for b in range(B):
        ...
        column_lookup = {col.item(): idx for idx, col in enumerate(cols)}
        ...
        while remaining_rows and remaining_cols:
            ...
            for r in remaining_rows:
                ...
                for idx, col_val in enumerate(remaining_cols):
                    col_idx = column_lookup[col_val]
                    val = row_vals[col_idx].item()
